...
import { getEventCategories } from '../../utils/calendarUtils';
...
function AddEventModal({ isOpen, onClose, onAdd, initialDate }: AddEventModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Set recurrence end date 1 month in future
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!title || !date || (!category && !newCategory)) {
    ...
    if (isRecurring && amount && parseFloat(amount) > 0) {
      setTimeout(() => {
